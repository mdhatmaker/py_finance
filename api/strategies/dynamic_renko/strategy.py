...
def plot_candlestick(ticker, df):
    ...
    mpf.plot(df, type='candle', style='charles', datetime_format='%Y-%m-%d', xrotation=20,
             title=str(ticker + ' Candlestick Chart'), ylabel='Price', xlabel='Date', scale_width_adjustment=dict(candle=2))
    

# Plot the Renko Data.
def plot_renko(ticker, renko_df):
    ...
    mpf.plot(renko_df, addplot=adp, type='candle', style='charles', datetime_format='%Y-%m-%d', xrotation=20,
             title=str(ticker + ' Renko Chart'), ylabel='Price', xlabel='Date', scale_width_adjustment=dict(candle=2))
...
if __name__ == "__main__":
    ...
    run_dynamic_renko(symbol, start_date, interval)

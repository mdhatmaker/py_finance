...
def get_historical_sentiment(symbol: str):
    #data_raw = DataGenerator.get_data(symbol)
    #data_raw.to_csv(f'{symbol}.csv')

    data = pd.read_csv(f'api/signals/sentiment_trading_signal/{symbol}.csv')
...
def analyze_strategy(ticker_symbol, data):
    ...
    quantstats.reports.html(returns, output='stats.html', title=f'{ticker_symbol} Sentiment')
    webbrowser.open('stats.html')


def run_sentiment_analysis_signal(symbol: str):
    data = get_historical_sentiment(symbol)
    analyze_strategy(symbol, data)


if __name__ == '__main__':
    run_sentiment_analysis_signal('GOOG')
